            ...
            <div class="flex-1">
                ...
                <div class="grid grid-cols-2 gap-12 mb-12">
                    ...
                    <div>
                        ...
                        <div class="space-y-1">
                            <p class="text-base font-bold text-gray-900">{{ $company['company_name'] ?? $tenant->name ?? 'RenewPilot Inc.' }}</p>
                            <p class="text-sm text-gray-600">{{ $company['address_line1'] ?? '123 Business Avenue, Suite 100' }}</p>
                            <p class="text-sm text-gray-600">
                                {{ $company['address_city'] ?? 'New York' }}{{ isset($company['address_state']) ? ', ' . $company['address_state'] : ', NY 10001' }}
                            </p>
                ...
                <div class="flex justify-end mb-16">
                    <div class="w-80 space-y-3 bg-gray-50 print-exact p-6 rounded-2xl border border-gray-100">
                        ...
                        <div class="flex justify-between text-sm pt-2">
                            ...
                            <span class="font-bold text-emerald-600">-{{ $symbol }}{{ number_format($invoice->amount_paid, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center pt-4 border-t border-gray-200 mt-2">
                            ...
                            <span class="text-3xl font-black text-indigo-600">{{ $symbol }}{{ number_format($invoice->total - $invoice->amount_paid, 2) }}</span>
                        </div>
                        @else
                        <div class="flex justify-between items-center pt-4 border-t border-gray-200 mt-2">
                            <span class="text-xs font-black text-gray-900 uppercase tracking-widest">Total Due</span>
                            <span class="text-3xl font-black text-indigo-600">{{ $symbol }}{{ number_format($invoice->total, 2) }}</span>
                        </div>
                        @endif
                    </div>
                </div>
    ...
    <div id="payment-modal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-gray-100">
                ...
                <form action="{{ route('app.payments.store') }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    <input type="hidden" name="invoice_id" value="{{ $invoice->id }}">

                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Invoice Number</label>
                        <input type="text" value="{{ $invoice->invoice_number }}" readonly disabled
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" step="0.01" max="{{ $invoice->total - $invoice->amount_paid }}" value="{{ $invoice->total - $invoice->amount_paid }}" required
                               class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm font-bold text-gray-900">
                        <p class="text-[10px] text-gray-400 mt-1">Remaining balance: {{ $symbol }}{{ number_format((float)($invoice->total - $invoice->amount_paid), 2) }}</p>
                    </div>
