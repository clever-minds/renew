                        ...
                        @else
                            <select name="invoice_id" id="invoice-select" required onchange="handleInvoiceChange()"
                                ...
                                @foreach($invoices as $inv)
                                    ...
                                    @endphp
                                    <option value="{{ $inv->id }}" data-remaining="{{ $remaining }}">
                                        {{ $inv->invoice_number }} - {{ $inv->client_name }} (Total: ₹{{ number_format((float)$inv->total, 2) }}, Bal: ₹{{ number_format((float)$remaining, 2) }})
                                    </option>
                                @endforeach
                            </select>
                        @endif
