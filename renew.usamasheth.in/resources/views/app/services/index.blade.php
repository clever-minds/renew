<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('app.services.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded shadow">
                + New Service
            </a>
        </div>
    </x-slot>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($services as $service)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ucfirst($service->billing_cycle->value) }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('app.services.show', $service) }}" class="text-indigo-600 hover:text-indigo-900 mr-4">View</a>
                                        <a href="{{ route('app.services.edit', $service) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                        <form action="{{ route('app.services.destroy', $service) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
